...
function nextOvernightStart() {
  const now = new Date();
  if (inOvernightWindow()) return now.toISOString();
...
async function runJob(db, sseBus, job) {
  ...
  try {
    ...
    console.log(`[transcode] Downloading ${file.r2_key}`);
    ...
    // Run ffmpeg: H.264, max 1920px wide, frame-rate pass-through, audio copy
    console.log(`[transcode] Running ffmpeg for job ${job.id}`);
    await new Promise((resolve, reject) => {
    ...
    console.log(`[transcode] Job ${job.id} done`);
