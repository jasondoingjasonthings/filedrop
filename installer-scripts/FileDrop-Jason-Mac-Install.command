#!/bin/bash
# FileDrop — Jason (Sender) macOS Installer
...
ARCH="$(uname -m)"
if [ "$ARCH" = "arm64" ]; then
  BIN_NAME="FileDrop-Jason-mac-arm64"
else
  BIN_NAME="FileDrop-Jason-mac-x64"
fi
...
echo "Installing FileDrop (Jason / Sender)..."
echo "Architecture: $ARCH"
echo "Install path: $INSTALL_DIR"
echo ""

mkdir -p "$INSTALL_DIR"
cp "$BIN_SRC" "$INSTALL_DIR/FileDrop-Jason"
chmod +x "$INSTALL_DIR/FileDrop-Jason"

# Register as a launchd service (auto-start on login)
"$INSTALL_DIR/FileDrop-Jason" --install
